...
function handleViewer(ws, room, auth) {
  ...
  ws.on('message', (data, isBinary) => {
    ...
    if (msg.type === 'prank') {
      // Repassa a pegadinha para todo mundo na sala
      const envelope = {
        type: 'prank',
        action: msg.action,
        from: auth.uid,
      ...
      for (const v of room.viewers) {
        R.sendJson(v, envelope);
      }
      // Envia para broadcasters (a aba de controle/transmissão pode estar conectada, mas os efeitos visuais rodam na main page que é um viewer)
    }
  });
